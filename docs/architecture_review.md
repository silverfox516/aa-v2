...
- **영향**:
  - PLAYBACK_INPUT proto는 docs/aap_messages.md에 catalog만 유지
    (필드 해석 자체는 학습 가치 있음, 구현은 보류).
...
- **얻은 학습**:
  ...
  - lag 영향 없음 — passive handler가 G.0의 "처리되고 있는가" 신호
    충족
- **남은 학습 (트리거 시 추가)**:
  - PLAYBACK_INPUT outbound 송신과 KEYCODE 경로의 응답 차이 비교
    (현재는 KEYCODE만 사용하므로 PLAYBACK_INPUT의 실제 응답 패턴은
    미관찰)
