...
### G.1 MediaBrowserService (channel 12) — 재확인 ON HOLD (2026-04-28)

- **현재**: 클래스 존재 + 4개 inbound 핸들러 + 2개 outbound primitive
  완비. **registration 제거됨** (2026-04-28, 폰이 채널 안 엶).
- **2026-04-28 Day 1 시도 결과**:
  - 풀 핸들러 set + auto-request hook + advertise 추가
  - 폰(Nothing A001 / SM-N981N)이 ServiceDiscoveryResponse를 받고도
    ch12 CHANNEL_OPEN_REQ를 송신하지 않음
  - YouTube Music + 추가 미디어 앱 모두 동일 결과
  - G.0 throttle 영향은 **없음** (advertise만으로 폰이 throttle 걸지는
    않음 — 채널 자체를 열지 않을 뿐)
  - 따라서 ROOT_NODE/SOURCE_NODE/LIST_NODE/SONG_NODE 어떤 응답도
    관찰 못 함
- **추정 원인 (검증 미진행)**:
  1. `MediaBrowserService { }`가 빈 message — 폰이 capability 필드
     (예: supported source types)를 기대해서 silent reject. 가장 유력.
  2. HeadunitInfo manufacturer/model 화이트리스트 게이팅.
  3. Spotify/YT Music이 차량 인증 절차(Driving Mode 등록 등)를 거치지
     않은 상태라 폰 측이 browse를 expose 안 함.
- **재시도 트리거**:
  - reference openauto/aasdk의 MediaBrowserService 설정과 우리 fill_config
    diff 확인. 추가 필드 발견 시 즉시 재시도.
  - 다른 폰 모델(Pixel 등)에서 같은 결과인지 확인.
  - cluster display 작업 등 다른 멀티 출력 시나리오와 함께 재검토.
- **유지된 자산**: MediaBrowserService.{hpp,cpp} 전체 — 재시도 시
  proto wiring + handler skeleton 재유도 비용 zero. 자세한 시도 기록은
  docs/plans/0005_media_browser.md.
